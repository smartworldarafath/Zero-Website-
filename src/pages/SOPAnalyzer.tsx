...
export function SOPAnalyzer() {
  const { sopAnalyzer, setSopAnalyzer } = useWork();
  const { sopText, programDetails, result } = sopAnalyzer;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setSopAnalyzer({ sopText: '' }); // Clear text if file is uploaded
    }
  };
  ...
  const handleGenerate = async () => {
    if (!sopText && !file) return;
    setLoading(true);
    try {
      let fileParts: any[] = [];
      let promptText = `Analyze this Statement of Purpose.\n\nTarget Program/University Details (if any):\n${programDetails}`;
      ...
      const textToCount = file ? (await parseFile(file)).text || '' : sopText;
      const wordCount = textToCount.trim().split(/\s+/).filter(word => word.length > 0).length;
      
      setSopAnalyzer({ result: { ...parsedResult, wordCount } });
    } catch (error: any) {
      console.error('Analysis error:', error);
      let errorMessage = error.message || 'Failed to analyze SOP.';
  ...
  const handleGenerateImproved = async () => {
    if (!result || (!sopText && !file)) return;
    setGeneratingImproved(true);
    try {
      let fileParts: any[] = [];
      let promptText = `Rewrite and improve this Statement of Purpose based on the previous analysis. Make it persuasive, clear, and tailored to the program details if provided.\n\nTarget Program/University Details:\n${programDetails}`;
      ...
      if (file) {
        ...
        if (parsed.inlineData) {
          ...
          promptText = `Original SOP Content:\n${parsed.text}\n\n` + promptText;
        }
      } else {
        promptText = `Original SOP Content:\n${sopText}\n\n` + promptText;
      }

      const systemInstruction = 'You are an expert admissions consultant and writer. Rewrite the provided Statement of Purpose to be perfect, persuasive, and highly effective. Return ONLY the improved SOP text in Markdown format.';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      <div className="space-y-2 text-center lg:text-left">
        <h1 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-200 tracking-tight">SOP Analyzer</h1>
        <p className="text-gray-800 dark:text-gray-200 text-lg font-medium">Upload your SOP (PDF, Image, Word) or paste text to get actionable feedback.</p>
      </div>
        ...
        <div className="space-y-6 liquid-panel p-8 rounded-[2rem]">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-900 dark:text-white mb-3">
              Upload SOP File (PDF, Word, Image)
            </label>
          ...
          <div>
            <textarea
              ...
              rows={6}
              placeholder="Ever since I was young, I have been fascinated by..."
              value={sopText}
              onChange={(e) => { setSopAnalyzer({ sopText: e.target.value }); if (e.target.value) clearFile(); }}
              disabled={!!file}
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-bold text-gray-900 dark:text-white mb-3">
              Target Program & University (Optional)
            </label>
            <textarea
              className="w-full rounded-2xl liquid-input p-4 text-sm"
              rows={3}
              ...
              onChange={(e) => setSopAnalyzer({ programDetails: e.target.value })}
            />
          </div>

          <button
            onClick={handleGenerate}
            disabled={(!sopText && !file) || loading}
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="h-full flex items-center justify-center">
                <GlowingTimer isRunning={loading} label="Analyzing SOP..." />
              </div>
            ) : generatingImproved ? (
              <div className="h-full flex items-center justify-center">
              ...
              <div className="h-full flex items-center justify-center text-gray-500 dark:text-gray-400 text-sm font-medium">
                Your SOP analysis will appear here.
              </div>
            )}
          </div>
        </div>
